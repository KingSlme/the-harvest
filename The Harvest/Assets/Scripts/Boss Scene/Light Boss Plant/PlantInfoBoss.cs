...
public class PlantInfoBoss : MonoBehaviour
{
    ...
    bool AllSeedsClicked() {
        ...
        foreach(GameObject o in seedList) {
            if(o.GetComponent<ClickSeedBoss>().CheckIfClicked() == false) {
                return false;
            }
        }
        return true;
    ...
    IEnumerator DelayDestroy() {
        ...
        foreach(GameObject o in seedList) {
                    o.GetComponent<ClickSeedBoss>().SetClickStatus();
                    // Regen
                    Destroy(o);
                }
        // New Clear:
    ...
    {   
        ...
        if(time > 2) {
            if(AllSeedsClicked() && !allClickedEventBegan && !GetComponentInChildren<PlantDeathBoss>().hasAlreadDied) {
                // Regen
