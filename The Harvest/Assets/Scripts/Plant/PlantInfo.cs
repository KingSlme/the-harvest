...
public class PlantInfo : MonoBehaviour
{
    ...
    bool AllSeedsClicked() {
        ...
        foreach(GameObject o in seedList) {
            if(o.GetComponent<ClickSeed>().CheckIfClicked() == false) {
                return false;
            }
        }
        return true;
    ...
    IEnumerator DelayDestroy() {
        ...
        foreach(GameObject o in seedList) {
                    o.GetComponent<ClickSeed>().SetClickStatus();
                    // Regen
                    Destroy(o);
                }
        // New Clear:
    ...
    {   
        ...
        if(time > 2) {
            if(AllSeedsClicked() && !allClickedEventBegan && !GetComponentInChildren<PlantDeath>().hasAlreadDied) {
                // Regen
